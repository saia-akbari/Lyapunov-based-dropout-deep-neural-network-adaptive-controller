...
Number=100;
X_rand=0.25*(-1+2*rand(5,1,Number));


[e_DNN,ftilde_DNN,u_list_DNN,vecV_list_DNN,x_DNN,f_list_DNN,F_DNN_error_rms] = ...
    DNN_func(k,L,s,"tanh",L_in,L_out, L_vec,vecV,step_size,simtime,x,ke,ks,Gamma, X_rand);
[e_RDNN,ftilde_RDNN,u_list_RDNN,vecV_list_RDNN,x_RDNN,f_list_RDNN,F_RDNN_error_rms]=...
    RDNN_func(k,n,L,s,2,deltat, "tanh",L_in,L_out, L_vec,vecV,step_size,simtime,x,ke,ks,Gamma, X_rand);
...
figure(1)
    ...
    ylabel('$||e||$','Fontsize',16,'Interpreter','latex')
    legend('DNN','Dropout DNN','Interpreter','latex','Fontsize',16,'Location','NorthEast','Orientation','Horizontal')
        
    set(gca, 'YLim', [-0.5 8.5], 'YTick', 0:1:8,...
     'YTickLabel', 0:1:8);
    ...
    ylabel('$||f(x)-\widehat{\Phi}||$','Fontsize',16,'Interpreter','latex')
    xlabel('Time (s)')
    legend('DNN','Dropout DNN','Interpreter','latex','Fontsize',16,'Location','NorthEast','Orientation','Horizontal')
...
figure(4)
    plot(time(1:1000), F_DNN_error_rms,'LineWidth',1.5)
    hold on
    plot(time(1:1000), F_RDNN_error_rms,'LineWidth',1.5)
    ylabel('RMS $||f(x)-\widehat{\Phi}||$ on test dataset','Fontsize',16,'Interpreter','latex')
    xlabel('Time (s)','Fontsize',16,'Interpreter','latex')
    legend('DNN', 'DDNN','Fontsize',16,'Interpreter','latex')
    set(gca, 'YLim', [-5 255], 'YTick', 0:25:250,...
     'YTickLabel', 0:25:250);
    set(gca, 'XLim', [-0.2 10.2], 'XTick', 0:1:10,...
     'XTickLabel', 0:1:10);
